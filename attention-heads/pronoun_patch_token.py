...
pronoun_patch_token.py  (repetition variant)

Use:
python pronoun_patch_token.py  --models meta-llama/Llama-3.1-8B-Instruct
python pronoun_patch_token.py  --models meta-llama/Llama-3.1-8B-Instruct --max_n 50
"""
...
_CACHE_ROOT = os.path.join(_REPO_ROOT, 'cache', 'repetition')
...
def evaluate_model(model_name, df):
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc=model_name.split('/')[-1]):
        ...
        clean_ids = tokenize_prefix(str(row['p_clean']),   tokenizer)
        corr_ids  = tokenize_prefix(str(row['p_corrupt']), tokenizer)
...
def build_parser():
    ...
    parser.add_argument('--data_path',
                        default=os.path.join(_REPO_ROOT, 'data',
                                             'eo_ep_task_repetition_pairs.tsv'))
