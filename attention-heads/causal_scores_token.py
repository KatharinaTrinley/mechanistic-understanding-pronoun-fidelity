...
causal_scores_token.py

basically same as causal_scores_concept.py.
...
Use:
python causal_scores_token.py --model google/gemma-2-9b-it --max_n 50
"""
...
def collect_clean_acts(model, input_ids):
    ...
    # We reshape by head_dim later, so the real tensor width sets the head count.
...
def main(args):
    ...
    # We don't know the total head count until we see the first activation
